...
void main(void)
{
    ...
    float noise = 10.0 *  -.10 * turbulence( .5 * vNormal + time );
    float b = 5.0 * snoise( 0.05 * vPosition.xyz + vec3( 2.0 * time ) );
    float displacement = - noise + b;
    
    vPosition = ciModelView * ciPosition;
    
    vec3 newPosition = vec3(ciPosition.xyz + vNormal * displacement/10.);
