...
void Day50App::mouseDown( MouseEvent event )
{
}

void Day50App::update()
{
    ...
    for (int i = 0; i < NUM_LIGHTS; i++)
    {
        mPointLights[i].x = 8 * cos(theta + i/NUM_LIGHTS);
        mPointLights[i].y = -5 + i;
        mPointLights[i].z = 8  * sin(theta + i/NUM_LIGHTS);
...
void Day50App::loadObj(const DataSourceRef &dataSource)
{
...
CINDER_APP( Day50App, RendererGl(RendererGl::Options().msaa(16)),
