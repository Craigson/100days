...
void Day68App::keyDown( KeyEvent event )
{
    if (event.getChar() == 'p') process = !process;
}


void Day68App::updateFbo()
{
    ...
    tempCam.lookAt(vec3(0,0,12), vec3(0));
...
void Day68App::mouseDown( MouseEvent event )
{
}

void Day68App::update()
{
...
void Day68App::saveGif()
{
    if (getElapsedFrames() % 4 == 0 && getElapsedFrames() < 361) writeImage( getDocumentsDirectory() / "Cinder" / "screenshots"/ "100days" / "68"/ ("68-0" + toString( getElapsedFrames() ) + ".png" ), copyWindowSurface() );
    
    cout << "saving image" << endl;
}


CINDER_APP( Day68App, RendererGl(RendererGl::Options().msaa(16)),
