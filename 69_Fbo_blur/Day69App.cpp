...
void Day69App::setup()
{
    ...
    mIcos = gl::Batch::create(geom::TorusKnot() >> geom::Scale(0.6,0.6,0.6), shader);
...
void Day69App::keyDown( KeyEvent event )
{
    if (event.getChar() == 'p') process = !process;
}


void Day69App::updateFbo()
{
    ...
    tempCam.lookAt(vec3(0,0,8), vec3(0));
    gl::setMatrices(tempCam);
    
    //gl::rotate(angleAxis(angleRot/2, normalize(vec3(0.,4.5,1.7))));
...
void Day69App::mouseDown( MouseEvent event )
{
}

void Day69App::update()
{
    
    mGlsl->uniform("uTime", 200 * fabs(cos(angleRot/2)));
    angleRot += 0.0523;
    updateFbo();
...
void Day69App::saveGif()
{
    if (getElapsedFrames() % 2 == 0 && getElapsedFrames() < 181) writeImage( getDocumentsDirectory() / "Cinder" / "screenshots"/ "100days" / "69"/ ("69-0" + toString( getElapsedFrames() ) + ".png" ), copyWindowSurface() );
    
    cout << "saving image" << endl;
}


CINDER_APP( Day69App, RendererGl(RendererGl::Options().msaa(16)),
