...
void Day10aApp::setup()
{
    ...
    auto scale = geom::Scale(1,0.02,1);
    ...
    for (int i = 0; i < NUM_COLS; i++)
    {
        ...
        {
            mCubes[i][j] = gl::Batch::create(geom::Cylinder() >> scale >> rotate >> color, shader);
        }
    }
...
void Day10aApp::draw()
{
    ...
    for (int i = 0; i < NUM_COLS; i++)
    {
        ...
        for (int j = 0; j < NUM_ROWS; j++)
        {
            ...
            float z = pow(2.7182817, -dist/50) * sin( (dist/10)-t)*1.5;
...
{
    if (getElapsedFrames() % 4 == 0 && getElapsedFrames() < 121) writeImage( getDocumentsDirectory() / "Cinder" / "screenshots"/ "100days" / "11"/ ("11-0" + toString( getElapsedFrames() ) + ".png" ), copyWindowSurface() );
