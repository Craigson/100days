...
void Day44App::mouseDown( MouseEvent event )
{
}
...
void Day44App::setupGlsl()
{
    mGlsl = gl::GlslProg::create(loadAsset("vertex.vert"), loadAsset("fragment.frag"));
    
}

void Day44App::saveGif()
{
    if (getElapsedFrames() % 2 == 0 && getElapsedFrames() < 481) writeImage( getDocumentsDirectory() / "Cinder" / "screenshots"/ "100days" / "44"/ ("44-0" + toString( getElapsedFrames() ) + ".png" ), copyWindowSurface() );
    cout << "saving image" << endl;
}

CINDER_APP( Day44App, RendererGl(RendererGl::Options().msaa(16)),
